...
function dispatcher(opt) {
  var commands = pre.extend({}, opt.commands),
      patterns = [];
  ...
  function match(client, msg, target, text) {
    ...
    function reply() {
      ...
      args.splice(0, 0, target, msg.nick + ':');
  ...
  if (!commands.help)
    match.command('help', 'List available commands', function(msg) {
      msg.reply('I know about these commands:');
      for (var name in commands) {
        msg.reply(' ', name, '--', commands[name].description);
      }
    });
