...
Stream.prototype.receive = function(msg) {
  var event = 'message',
      cmd = msg.command;
...
Stream.prototype.sendReceive = function(command) {
...
//   + secret(target, text, ...) -- send a message without emiting it
...
Client.prototype.sendMessage = function(method, target, text) {
    var // The maximum message size is 512, subtract:
        //   * 8 for 'PRIVMSG '
        //   * target.length
        //   * 2 for ' :'
        //   * 2 for '\r\n'
    ...
    while (text.length > limit) {
        method.call(this, 'PRIVMSG', target, text.substr(0, limit));
        text = text.substr(limit, text.length);
    }
    return method.call(this, 'PRIVMSG', target, text);
};

Client.prototype.privmsg = function(target) {
  var text = Array.prototype.slice.call(arguments, 1).join(' ');
  return this.sendMessage(this.sendReceive, target, text);
};

Client.prototype.secret = function(target) {
  var text = Array.prototype.slice.call(arguments, 1).join(' ');
  return this.sendMessage(this.send, target, text);
};

Client.prototype.identify = function(nick, password) {
    return this.secret('NickServ', 'IDENTIFY', nick, password);
};

Client.prototype.ghost = function(nick) {
    if (this._nick != nick)
        this.secret('NickServ', 'GHOST', nick).nick(nick);
    return this;
},
